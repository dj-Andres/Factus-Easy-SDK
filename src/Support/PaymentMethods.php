<?php

namespace FactusEasy\Sdk\Support;

final class PaymentMethods
{
    public const EFECTIVO = '01';

    public const COMPENSACION = '15';

    public const TARJETA_DEBITO = '16';

    public const DINERO_ELECTRONICO = '17';

    public const TARJETA_PREPAGO = '18';

    public const TARJETA_CREDITO = '19';

    public const OTROS = '20';

    public const ENDOSO = '21';

    public const COBRO = '22';

    public const PAYPAL = '23';

    public const TRANSFERENCIA = '24';

    public const CHEQUE = '25';
}
